# E3-15 Execution Spec

## Objective

- Deliver E3-15 (HMAC key generation).
- Backlog contract: At `register-agent` time, generate a random 256-bit HMAC key in `secrecy::Secret`, encrypt via Secure Enclave P-256 key (`SecKeyCreateEncryptedData`), store as `hmac.enc` in agent directory, wipe plaintext from memory.
...
- In scope:
  - At `register-agent` time, generate a random 256-bit HMAC key in `secrecy::Secret`, encrypt via Secure Enclave P-256 key (`SecKeyCreateEncryptedData`), store as `hmac.enc` in agent directory, wipe plaintext from memory.
- Out of scope:
  - Unrelated backlog items outside E3-15
  - Cross-epic behavior changes not requested by E3-15

## Acceptance Criteria

- AC1: E3-15 behavior matches backlog description: At `register-agent` time, generate a random 256-bit HMAC key in `secrecy::Secret`, encrypt via Secure Enclave P-256 key (`SecKeyCreateEncryptedData`), store as `hmac.enc` in agent directory, wipe plaintext from memory.
...
- [x] docs/plan/spec/PKT-E3-15-work-item.md
