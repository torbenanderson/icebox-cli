# E3-08 Execution Spec

## Objective

- Deliver E3-08 (`mlock` pinning).
- Backlog contract: Secret buffers are `mlock`'d to prevent paging to swap/disk
...
- In scope:
  - Secret buffers are `mlock`'d to prevent paging to swap/disk
- Out of scope:
  - Unrelated backlog items outside E3-08
  - Cross-epic behavior changes not requested by E3-08

## Acceptance Criteria

- AC1: E3-08 behavior matches backlog description: Secret buffers are `mlock`'d to prevent paging to swap/disk
...
- [x] docs/plan/spec/PKT-E3-08-work-item.md
