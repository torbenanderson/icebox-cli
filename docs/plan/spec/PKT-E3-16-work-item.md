# E3-16 Execution Spec

## Objective

- Deliver E3-16 (Vault HMAC on write).
- Backlog contract: On every vault write (`add`, `remove`): unwrap `hmac.enc` via enclave, compute `HMAC-SHA256(hmac_key, vault_body)` (body excludes the `hmac` field), store tag as hex string in `vault.enc` `hmac` field, wipe HMAC key.
...
- In scope:
  - On every vault write (`add`, `remove`): unwrap `hmac.enc` via enclave, compute `HMAC-SHA256(hmac_key, vault_body)` (body excludes the `hmac` field), store tag as hex string in `vault.enc` `hmac` field, wipe HMAC key.
- Out of scope:
  - Unrelated backlog items outside E3-16
  - Cross-epic behavior changes not requested by E3-16

## Acceptance Criteria

- AC1: E3-16 behavior matches backlog description: On every vault write (`add`, `remove`): unwrap `hmac.enc` via enclave, compute `HMAC-SHA256(hmac_key, vault_body)` (body excludes the `hmac` field), store tag as hex string in `vault.enc` `hmac` field, wipe HMAC key.
...
- [x] docs/plan/spec/PKT-E3-16-work-item.md
