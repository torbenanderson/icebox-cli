...
- Skill order: If all apply, use `icebox-docs-standards` -> `icebox-load` -> `icebox-execute` -> `icebox-test` -> `icebox-ai-harness` -> `icebox-done-gate` -> `icebox-commit-merge-hygiene`. Otherwise, preserve that relative order for whichever subset applies.
